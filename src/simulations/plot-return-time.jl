...
df_tau = combine(groupby(df_tau, [:species, :ry, :alpha]), :tau => mean)
...
for (i, gdf) in enumerate(groupby(df_tau, :alpha))
    ...
    ax_list[i] =
        ax = Axis(
            ...
            ylabel = i == 1 ? "Return rate" : "",
            title = title[i] * " (t = $alpha)",
        )
    scatter!(gdf.ry, gdf.tau_mean; color = :grey, label = "simulation")
end
ax = Axis(
    g2[1, 1];
    xlabel = "Time",
...
for (i, row) in enumerate(eachrow(xi_mean))
    lines!(time_steps, row; color = ry[i], colorrange = extrema(ry))
end
cb = Colorbar(g2[1, 3]; limits = extrema(ry), label = "Relative yield")
cb.alignmode = Mixed(; right = 0)
ax = Axis(
    g2[1, 2];
    xlabel = "Mean relative\ndistance to equilibrium",
    ylabel = "Instantaneous \n return rate",
    # title = "Subordination to community",
)
poly!(Point2f[(-0.05, -0.1), (-0.05, 0.4), (0.15, 0.4), (0.15, -0.1)]; color = :lightgray)
for i in 1:S
    lines!(xi_com[1:end-1], xi_diff[i, 1:end]; color = ry[i], colorrange = extrema(ry))
end
...
for i in 1:S
    lines!(xi_com[a:end-1], xi_diff[i, a:end]; color = ry[i], colorrange = extrema(ry))
end
hlines!([0]; color = :black, label = "y = 0")
# axislegend(; position = :rb)

for (label, layout) in
    zip(["A", "B", "C", "D", "E"], [g2[1, 1], g2[1, 2], g1[1, 1], g1[1, 2], g1[1, 3]])
    Label(
        layout[1, 1, TopLeft()],
        label;
        font = :bold,
