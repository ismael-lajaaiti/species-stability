...
df = DataFrame(; p=Float64[], mu=Float64[], sp=Int64[], deltaB=Float64[], deltaK=Float64[], B=Float64[], K=Float64[])
df2 = DataFrame(; p=Float64[], mu=Float64[], sp=Int64[], xt=Float64[], x0=Float64[])
coms = []
...
df_com = combine(groupby(df, [:mu, :p]), [:B, :deltaB, :K, :deltaK] .=> sum, renamecols=false)
df_com = select(df_com, :, [:deltaB, :B] => ByRow(/) => :dB, [:deltaK, :K] => ByRow(/) => :dK)
...
cm = inch / 2.54
width = 17.8cm
fig = Figure(; size=(width, 0.5 * width), fontsize=10pt);
ax = Axis(fig[1, 1], xlabel="Mean interaction strength, μ", ylabel="Sensitivity to press \n (reversed)")
ax.yreversed = true
scatter!(df_join.mu, df_join.s_com, label="community")
scatter!(df_join.mu, df_join.s_sp, label="species")
axislegend()
ax = Axis(fig[1, 2], xlabel="Mean interaction strength, μ", ylabel="Short-term recover rate")
scatter!(df_join2.mu, df_join2.r_com, label="community")
scatter!(df_join2.mu, df_join2.r_sp, label="species")
fig

save("figures/simulations/si-sp-vs-com.svg", fig)
